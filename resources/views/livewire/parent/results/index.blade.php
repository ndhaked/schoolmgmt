...
new #[Layout('components.layouts.panel')] class extends Component
{
    public function with(): array
    {
        ...
        $children = $parent
            ? $parent->students()->with(['user', 'schoolClass'])->get()
            : collect();

        $resultsByChild = $children->mapWithKeys(function ($child) {
            $attempts = ExamAttempt::with(['exam.subject', 'exam.questions'])
...
<div>
    <x-slot name="header">
        <h1 class="text-lg font-semibold text-gray-900">Child's Results</h1>
    </x-slot>

    @forelse ($children as $child)
        <div wire:key="child-{{ $child->id }}" class="mb-6">
            <div class="flex items-center justify-between mb-2">
                <h2 class="text-sm font-semibold text-gray-900">
                    {{ $child->user->name }} <span class="text-gray-400 font-normal">· {{ $child->schoolClass->name }}</span>
                </h2>
                <a href="{{ route('parent.marksheet', $child) }}" wire:navigate class="text-sm font-medium text-indigo-600 hover:text-indigo-800">
                    View Marksheet
                </a>
            </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($resultsByChild[$child->id] as $attempt)
                            @php
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-8 text-center text-gray-500">
                                    No results declared yet for {{ $child->user->name }}.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
